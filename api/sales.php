 ...
 * Version: 1.9 (Query Optimization: Summary Caching)
 */
...
switch ($method) {
    case 'GET': getSales(); break;
    case 'POST': createSale(); break;
    default:
...
function getSales() {
    ...
    if (isset($_GET['bill_no']) || isset($_GET['id'])) {
        try {
            $stmt = isset($_GET['bill_no']) 
                ? $pdo->prepare("SELECT * FROM sales WHERE bill_no = :bill_no")
                : $pdo->prepare("SELECT * FROM sales WHERE id = :id");
            
            $stmt->execute(isset($_GET['bill_no']) ? ['bill_no' => $_GET['bill_no']] : ['id' => intval($_GET['id'])]);
            $sale = $stmt->fetch();
            
            if (!$sale) { sendResponse('error', [], 'Bill not found'); http_response_code(404); return; }
    ...
    $where = [];
    $params = [];
    if (isset($_GET['date_from'])) { $where[] = "sale_date::date >= :date_from::date"; $params['date_from'] = $_GET['date_from']; }
    if (isset($_GET['date_to'])) { $where[] = "sale_date::date <= :date_to::date"; $params['date_to'] = $_GET['date_to']; }
    ...
    try {
        ...
        // OPTIMIZATION: Cache summary query for 60 seconds
        $isFiltered = isset($_GET['date_from']) || isset($_GET['date_to']);
        $summaryCacheKey = $isFiltered ? "sales:summary:filtered" : "sales:summary:all";
        $summary = cache_get($summaryCacheKey);
        
        if ($summary === false) {
            $summarySql = "SELECT COUNT(*) as bill_count, SUM(total_amount) as total_sales, SUM(discount) as total_discount, SUM(net_total) as total_net, SUM(profit) as total_profit FROM sales {$whereClause}";
            $summaryStmt = $pdo->prepare($summarySql);
            foreach ($params as $key => $value) $summaryStmt->bindValue($key, $value);
            $summaryStmt->execute();
            $summary = $summaryStmt->fetch();
            cache_set($summaryCacheKey, $summary, 60); // Cache 60 seconds
        }
        
        sendResponse('success', [
            'sales' => $sales, 'total' => (int)$total, 'limit' => $limit, 'offset' => $offset,
            'summary' => [
...
function createSale() {
    ...
    if (empty($data['items']) || !is_array($data['items'])) { sendResponse('error', [], 'Items are required and must be an array'); http_response_code(400); return; }
    ...
    try {
        ...
        $totalAmount = 0; $totalProfit = 0; $saleItems = [];
        
        foreach ($data['items'] as $item) {
            if (empty($item['product_id']) || !isset($item['quantity'])) throw new Exception('Each item must have product_id and quantity');
            
            $productId = intval($item['product_id']);
            $quantity = floatval($item['quantity']);
            if (!$isVoid && $quantity <= 0) throw new Exception('Quantity must be greater than 0');
            ...
            $saleItems[] = [
                'product_id' => $productId, 'product_name_snapshot' => $product['name'], 'barcode_snapshot' => $product['barcode'],
                'quantity' => $quantity, 'price_snapshot' => $price, 'cost_snapshot' => $cost, 'subtotal' => $subtotal,
                'no_stock_count' => $noStockCount, 'current_stock' => intval($product['stock'])
            ];
        }
        ...
        if (!in_array($paymentMethod, ['cash', 'transfer', 'other'])) throw new Exception('Invalid payment method');
        
        $saleSql = "INSERT INTO sales (bill_no, total_amount, discount, net_total, payment_method, profit, original_bill_no)
                    VALUES (:bill_no, :total_amount, :discount, :net_total, :payment_method, :profit, :original_bill_no) RETURNING id";
        $saleStmt = $pdo->prepare($saleSql);
        $saleStmt->execute(['bill_no' => $billNo, 'total_amount' => $totalAmount, 'discount' => $discount, 'net_total' => $netTotal, 'payment_method' => $paymentMethod, 'profit' => $totalProfit, 'original_bill_no' => $originalBillNo]);
        $saleId = $saleStmt->fetch()['id'];
        ...
        foreach ($saleItems as $item) {
            ...
            $pdo->prepare($itemSql)->execute(['sale_id' => $saleId, 'product_id' => $item['product_id'], 'product_name_snapshot' => $item['product_name_snapshot'], 'barcode_snapshot' => $item['barcode_snapshot'], 'quantity' => $item['quantity'], 'price_snapshot' => $item['price_snapshot'], 'cost_snapshot' => $item['cost_snapshot'], 'subtotal' => $item['subtotal']]);
            
            if (!$item['no_stock_count']) {
                $pdo->prepare("UPDATE products SET stock = stock - :quantity WHERE id = :id")->execute(['quantity' => $item['quantity'], 'id' => $item['product_id']]);
                ...
                $pdo->prepare($logSql)->execute(['product_id' => $item['product_id'], 'change_type' => $changeType, 'quantity_change' => -$item['quantity'], 'reason' => $reason, 'reference_id' => $saleId, 'current_stock' => $item['current_stock']]);
            }
        }
        
        $pdo->commit();
        
        // OPTIMIZATION: Clear sales summary cache on new sale
        cache_delete('sales:summary:all');
        cache_delete('sales:summary:filtered');
        
        $sale = $pdo->prepare("SELECT * FROM sales WHERE id = :id")->execute(['id' => $saleId']) ? $pdo->query("SELECT * FROM sales WHERE id = {$saleId}")->fetch() : null;
        $sale['total_amount'] = floatval($sale['total_amount']);
        $sale['discount'] = floatval($sale['discount']);
        $sale['net_total'] = floatval($sale['net_total']);
        $sale['profit'] = floatval($sale['profit']);
        ...
        sendResponse('success', $sale, $isVoid ? 'Void bill created successfully' : 'Sale created successfully');
