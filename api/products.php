...
function createProduct() {
    ...
    if (empty($data['name']) || empty($data['barcode'])) {
        sendResponse('error', [], 'Name and barcode are required');
        http_response_code(400);
        return;
    }
    
    try {
        $checkSql = "SELECT id FROM products WHERE barcode = :barcode";
        $checkStmt = $pdo->prepare($checkSql);
        $checkStmt->execute(['barcode' => $data['barcode']]);
        
        if ($checkStmt->fetch()) {
            sendResponse('error', [], 'Barcode already exists');
            http_response_code(400);
            return;
        }
        ...
        // ถ้า no_stock_count = true ให้ stock = 0 เสมอ
        if ($no_stock_count) {
            $stock = 0;
        }
        
        $sql = "INSERT INTO products (name, barcode_status, barcode, price, cost, stock, unit, category, category_id, no_stock_count)
                VALUES (:name, :barcode_status, :barcode, :price, :cost, :stock, :unit, :category, :category_id, :no_stock_count)
                RETURNING id";
        ...
        $stmt->execute([
            ...
            'no_stock_count' => $no_stock_count ? 1 : 0
        ]);
...
function updateProduct() {
    ...
    try {
        ...
        // จัดการ stock และ no_stock_count พร้อมกัน
        $noStockCount = null;
        if (isset($data['no_stock_count'])) {
            $noStockCount = convertToBoolean($data['no_stock_count']);
        }
        
        if (isset($data['stock'])) {
            if ($noStockCount === true) {
                $updates[] = "stock = 0";
            } else {
                $updates[] = "stock = :stock";
                $params['stock'] = intval(convertEmptyToZero($data['stock']));
            }
        } elseif ($noStockCount === true) {
            $updates[] = "stock = 0";
        }
        ...
        if ($noStockCount !== null) {
            $updates[] = "no_stock_count = :no_stock_count";
            $params['no_stock_count'] = $noStockCount ? 1 : 0;
        }
